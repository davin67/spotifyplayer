...
The SDK relies on the experimental plugin system of Next.js. This prevents the SDK from running on the dev server (though it always runs in the client), and means changes to the plugin system may prevent the SDK from working as expected. We recognize the irony.
