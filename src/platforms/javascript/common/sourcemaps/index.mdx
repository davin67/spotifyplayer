...
Next you will need to generate an access token for our API. Within your organization's settings navigate to Developer Settings and [create a new internal integration](/product/integrations/integration-platform/#internal-integrations). You can fill in whatever you want for the name, but it's important that you select **Releases -> Admin** for Permissions.
...
From there, you will need to install and configure [sentry-cli](/cli/configuration/). Use the token you just created when configuring the CLI:
